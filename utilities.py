import numpy as np
import pandas as pd
labels = pd.read_csv('https://raw.githubusercontent.com/fjzs/Segmentor/main/datasets/pascal/pascal_segmented_classes_per_image.csv', index_col=1).drop('Unnamed: 0', axis = 1)#pd.read_csv('pascal_segmented_classes_per_image.csv', index_col='image_filename')
...
def iou_m(model, image_for_prediction): #can change the model
  ...
  # model = 'modelDeepLabV3_Mila.tflite'
...
def iou_per_class(model, image_for_prediction): #can change the model
  image_name = image_for_prediction
  import tensorflow as tf
  from PIL import Image
  import matplotlib.pyplot as plt
